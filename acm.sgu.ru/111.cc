...
typedef struct { unsigned n, a[127]; } num_t;
...
void div(num_t *q, num_t *r, num_t *x, num_t *y)
{
	...
	for (rp = r->n - 1; qp--; rp--) {
		...
		for (c = 0, i = qp, j = 0; j <= y->n; i++, j++) {
			c += m * (u64)y->a[j];
			if (r->a[i] >= (unsigned)(c & 0xFFFFFFFF)) {
				r->a[i] -= (unsigned)(c & 0xFFFFFFFF);
				c >>= 32;
			} else {
				r->a[i] += (unsigned)(0x100000000 - (c & 0xFFFFFFFF));
				c = (c >> 32) + 1;
			}
		}
