...
int down[128][128], right[128][128], seen[128][128], n;
...
{int y,x;printf("----------------------\n");
  ...
  printf("\n");};}
	...
	qy[0] = C[n].y1;
	qx[0] = C[n].x1;
	seen[C[n].y1][C[n].x1] = 0;

	for (; qh < qt && seen[C[n].y2][C[n].x2] < 0; qh++) {
		k = seen[qy[qh]][qx[qh]] + 1;

		for (i = qy[qh], j = qx[qh]; right[i][j] == 0;) {
			j++;
			if (seen[i][j] < 0)
				seen[i][j] = k, qy[qt] = i, qx[qt] = j, qt++;
		}
