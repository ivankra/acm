/* implements only paritioning into two parts; of couse gets WA */
#include <stdio.h>
#include <string.h>
#include <assert.h>
...
cost(M_i ... M_{i+k-1}) = R[i](R[i+1] R[i+2] + R[i+2] R[i+3] + ... + R[i+k-1] R[i+k])
...
/*fixme: partition into sevral parts?*/
	}
...
/*printf("total sequential cost: %d\n", cost[0][n]);*/
}
