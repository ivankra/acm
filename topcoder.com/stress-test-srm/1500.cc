...
struct Response {
	int response(vector<int> period, vector<int> execution) {
		int N = period.size();
		int done[51], avail[51];
		for (int i = 0; i < N; i++) done[i] = avail[i] = 0;

		for (int t = 0; ;) {
			for (int i = 0; i < N; i++)
				if (t % period[i] == 0) avail[i] += execution[i];
...
OutputType Execute(InputTuple input) {
	Response instance;
	return instance.response(input.arg1, input.arg2);
}
...
{
	const char *example[] = {
		"{20,2,27}, {4,1,8} Returns: 32",
"{8,8,8,8}  , {3,4,1,1} Returns: -1",
"{17}, {25} Returns: 25",
"{10,20,30,40,50}, {2,4,6,8,10} Returns: 76"
