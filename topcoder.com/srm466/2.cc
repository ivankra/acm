...
LD choose[512][512], memo[101][6];
char got[101][6];

LD f(int n, int k) {
	LD &res = memo[n][k];
	if (got[n][k]) return res;
	got[n][k] = 1;

	if (n == 0 || k < 3) return res = 0;

	res = 0;
	for (int s = 0; s <= 5 && s <= k; s++) {
		LD ps = choose[5*(n-1)][k-s] * choose[5][s] / choose[5*n][k];
		res += ps * ((s >= 3) ? 1 : (f(n-1, k-s)));
	}
...
OutputType Execute(InputTuple input) {
	LotteryPyaterochka instance;
	return instance.chanceToWin(input.arg1);
}
