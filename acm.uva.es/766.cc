...
struct Frac {
	...
	Frac(int n = 0, int d = 1) : num(n), den(d) { reduce(); }
	Frac(const BigInt &x, const BigInt &y) : num(x), den(y) { reduce(); }
...
template<typename T> T lcm(const T &a, const T &b) { return a / __gcd(a, b) * b; }
...
void gauss() {
	for (int i = 0; i < N; i++) {
		if (mat[i][i].isZero()) {
			...
			for (int j = i; j <= N; j++) swap(mat[k][j], mat[i][j]);
		}
		...
		for (int k = 0; k < N; k++) {
			if (i == k || mat[k][i].isZero()) continue;
