...
const BigInt &get(const poly &a, int k) {
	return k < (int)a.size() ? a[k] : zero;
}

void normalize(poly &p) {
	while ((int)p.size() > 0 && p.back() == 0) p.pop_back();
}
...
void print(poly p) {
	...
	for (int i = p.size()-1; i >= 0; i--) {
		...
		if (i != (int)p.size()-1 && p[i] > 0) printf("+");

		if (i == 0) {
			printf("%s", p[i].str().c_str());
		} else {
			if (p[i] != 1) printf("%s", p[i].str().c_str());
			if (i != 1)
				printf("Cos^%d(A)", i);
			else
				printf("Cos(A)");
...
int main() {
	poly Cos[60];
	poly SinS[60];    // Sin[n] = SinS[n] sin(a);
	...
	for (int n = 2; n <= 50; n++) {
		SinS[n] = SinS[n-1] * Cos[1] + Cos[n-1];
		if (n % 2 == 0) {
			Cos[n] = P("2") * Cos[n/2] * Cos[n/2] - P("1");
		} else {
			Cos[n] = Cos[n-1] * Cos[1] - SinS[n-1] * (P("1") - Cos[1] * Cos[1]);
		}
	}
