...
static void prep_t(struct fig *f)
{
	int i;
	...
	for (i = 1; i < 3; i++) {
		if (f->x[i] > f->maxx) f->maxx = f->x[i];
		else if (f->x[i] < f->minx) f->minx = f->x[i];

		if (f->y[i] > f->maxy) f->maxy = f->y[i];
		else if (f->y[i] < f->miny) f->miny = f->y[i];
	}
}
