...
int seen[512*512], prev[512*512], dist[512*512], prevc[512*512];
...
int solve() {
	...
	for (;;) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (c != box) {
				int t = (c << 9) | box;
				if (seen[t] == cs && dist[t] <= dist[s]+1) continue;
				seen[t] = cs;
				dist[t] = dist[s] + 1;
				prev[t] = s;
				prevc[t] = ds[d];
			...
			int t = (c << 9) | c1;
			if (seen[t] == cs && dist[t] <= dist[s] + 1000) continue;
			seen[t] = cs;
			dist[t] = dist[s] + 1000;
			prev[t] = s;
