...
Side sides[MAXN];
int nextSide[MAXN], prevSide[MAXN];
...
//			prevSide[j] = i;
		...
		while (N >= 3) {
			...
			for (int i = 0; i < N; i++) {
				const Side &prev = sides[(i+N-1) % N];
				const Side &cur = sides[i];
				const Side &next = sides[(i+1) % N];

				if (prev.ai * cur.bi == prev.bi * cur.ai) continue;
				if (cur.ai * next.bi == cur.bi * next.ai) continue;
				...
				double Qx = -(l.c * m.b - l.b * m.c) / det;
				double Qy = -(l.a * m.c - l.c * m.a) / det;
				double t = cur.a * Qx + cur.b * Qy + cur.c;
			...
			for (int j = minIndex+1; j < N; j++) sides[j-1] = sides[j];
			N--;
		}
