...
/*
char *bin(int x){static char b[8][16];static int i,z=0;for(z=(z+1)%8,i=0;i<
width;i++)b[z][i]=((x>>i)&1)+'0';b[z][width]=0;return b[z];}
*/
...
/*printf("** g transition: pvac'=%s\n", bin(pvac));*/
	...
	if (((pvac >> col) & 1) == 0) {
/*printf("[must move up  %c from r=%d c=%d]\n", src[row][col], row, col);*/
		/* must move up */
...
/*printf("[1+]\n");*/
	...
	if (col > 0 && dst[row][col-1] == src[row][col] &&
	    ((nvac >> (col - 1)) & 1) == 0) {
/*printf("[move left %c r=%d c=%d,   cost=%d]\n", src[row][col], row, col, ((nvac >> col) & 1));*/
...
{
	if (row == height) {
/*printf("f(row=%d,pdown=%s,pvac=%s) = %d\n", row, bin(pdown), bin(pvac),
(pdown == 0 && pvac == ((1 << width) - 1)) ? 0 : INF
);*/
...
/*printf("f(row=%d,pdown=%s,pvac=%s)\n", row, bin(pdown), bin(pvac));*/
	...
	tab[row][pdown][pvac] = g(row, pdown, pvac, 0, 0, 0);

/*printf("f(row=%d,pdown=%s,pvac=%s) = ", row, bin(pdown), bin(pvac));
if (tab[row][pdown][pvac] >= INF) printf("inf\n"); else printf("%d\n", tab[row][pdown][pvac]);*/
