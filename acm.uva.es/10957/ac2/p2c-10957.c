...
void print(sudoku_t *s)
{
	int i, j, a[16][16];

	for (i = 0; i < 81; i++)
		a[cells[i].row][cells[i].col] = s->a[i] ? ('0' + s->a[i]) : '.';

	for (i = 0;; i++) {
		if ((i % 3) == 0) printf("+-------+-------+-------+\n");
		if (i == 9) break;
		for (j = 0; j < 9; j++) printf("%s%c ", (j % 3) ? "" : "| ", a[i][j]);
		printf("|\n");
	}
...
int sread(sudoku_t *s)
{
	...
	s->invalid=0;
	for (i = 0; i < 81 && scanf("%d", &x) == 1; i++) {
		if (x < 0 || x > 9) s->invalid=1;
		if (x >= 1) place(s, i, x);
	}
...
int valid(sudoku_t *s)
{
	...
	for (g = 0; g < 27; g++) {
		for (i = 0; i < 9; i++) {
			for (j = i + 1; j < 9; j++) {
				a = s->a[grps[g].cells[i]];
				b = s->a[grps[g].cells[j]];
...
char *solve(sudoku_t *s)
{
	...
	if (nsol == 0) return "Impossible";
	if (nsol == 1) return "Unique";
	return "Ambiguous";
}
