...
int inside(hull_t *a, int x, int y)
{
	...
	if (a->n == 1)
		return (a->x[0] == x && a->y[0] == y);

	if (a->n == 2) {
		t = (a->x[0] - x) * (a->y[1] - y) - (a->x[1] - x) * (a->y[0] - y);
		if (t != 0) return 0;

		if (x < a->x[0] && x < a->x[1]) return 0;
		if (x > a->x[0] && x > a->x[1]) return 0;

		if (y < a->y[0] && y < a->y[1]) return 0;
		if (y > a->y[0] && y > a->y[1]) return 0;
...
int main()
{
	...
	for (t = 1; scanf("%d %d %d", &nc, &nr, &no) == 3; t++) {
		...
		while (no-- > 0 && scanf("%d %d", &x[0], &y[0]) == 2) {
			if (nc >= 3 && inside(&c, x[0], y[0]))
				s = "safe";
			else if (nr >= 3 && inside(&r, x[0], y[0]))
				s = "robbed";
			else
				s = "neither";
